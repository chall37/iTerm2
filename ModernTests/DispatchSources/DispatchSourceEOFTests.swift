...
/// Before the fix, pausing suspended the read source, so POLLHUP was never seen.
final class DispatchSourceEOFWhilePausedTests: XCTestCase {
    ...
    /// Regression: closing the write end while paused must still trigger brokenPipe.
    /// The read source stays active for EOF detection; handleReadEvent uses poll(POLLHUP)
    /// to probe for hangup when shouldRead is false.
    ...
    /// Regression: buffered data followed by EOF while paused must drain data
    /// before reporting brokenPipe.
