...
@objc class PTYTaskIOHandler: NSObject {
    ...
    /// Whether we are currently executing on the ioQueue.
    private var isOnIOQueue: Bool {
        let currentLabel = String(cString: __dispatch_queue_get_label(nil))
        let ioLabel = ioQueue.label
        return currentLabel == ioLabel
    }
