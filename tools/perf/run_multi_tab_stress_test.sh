...
Options:
  ...
  --dtrace       Enable enhanced DTrace UX metrics collection. Measures:
                 ...
                 Requires sudo access for DTrace.
...
# Validate stress modes if specified
if [[ -n "$stress_mode" ]]; then
  valid_modes="normal clearcodes buffer all"
...
# DTrace requires root - check if we can sudo
if [[ "$dtrace_mode" == true ]]; then
  ...
  if ! sudo -n true 2>/dev/null; then
    echo "Error: --dtrace requires sudo access. Either:" >&2
    echo "  1. Run this script with sudo" >&2
    echo "  2. Configure passwordless sudo for dtrace" >&2
    echo "  3. Run 'sudo -v' first to cache credentials" >&2
    exit 1
  fi
...
run_single_test() {
  ...
  if [[ "$synchronized_start" != true ]]; then
    ...
    if [[ "$dtrace_mode" == true ]]; then
      timeout "$profiler_duration" sudo dtrace -s "$dtrace_script" -p "$iterm_pid" > "$dtrace_output" 2>&1 &
      dtrace_pid=$!
    fi
    sleep 1
  fi
